...
public class GridDisplay extends JPanel implements MouseListener,
	...
	public void paint(Graphics g) {
		...
		final int squareEdgeSize = Math.min(
			...
			final StateEnum state, final boolean isTemporary ) 
	{
		...
		final int currenXPosition=    squareDisplayingBoat.getX();   
		final int boatSize=           boatBeingAdded.getLength();  
		final int boatCenter=         boatBeingAdded.getCenter();  
		final int boatRightEdgePosition = squareDisplayingBoat.getX()
				+ (boatSize - boatCenter);
		final int boatLeftEdgePosition = squareDisplayingBoat.getX()
				- boatCenter;
